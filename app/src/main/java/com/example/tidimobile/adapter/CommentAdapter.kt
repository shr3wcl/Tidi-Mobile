    ...
    RecyclerView.Adapter<CommentAdapter.CommentViewHolder>()
{
    ...
    override fun onBindViewHolder(holder: CommentViewHolder, position: Int) {
        holder.itemView.apply {
            findViewById<TextView>(R.id.nameUserCmt).text = "${listComment[position].idUser?.firstName} ${listComment[position].idUser?.lastName}"

            val rawDateFormat = SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss.SSSXXX")
            val date: Date = listComment[position].createdAt?.let { rawDateFormat.parse(it) } as Date
            ...
            Glide.with(this).load(decodedImage).diskCacheStrategy(
                DiskCacheStrategy.ALL).circleCrop().into(img)
